		...
		return new Promise((resolve, reject) => {
			...
			const cb = () => {
				if(this.banchojs.isConnected()) {
					this.banchojs.send("PRIVMSG "+name+" :"+this.message, false);
					if(this.recipient instanceof require("./BanchoUser"))
						this.banchojs.emit("PM", new PrivateMessage(this.recipient, this.message, true));
					else if(this.recipient instanceof require("./BanchoChannel"))
						this.banchojs.emit("CM", new ChannelMessage(this.banchojs.getSelf(), this.message, true, this.recipient));
					resolve();
				}
				else
