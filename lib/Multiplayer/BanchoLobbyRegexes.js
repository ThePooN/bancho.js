...
module.exports = {
	...
	regexes: {
		...
		timerEnded: function(str) {
			return str == "Countdown finished";
		},
		timerAborted: function(str) {
			return str == "Countdown aborted";
		},
		timerTick: function(str) {
			const r = /^Countdown ends in (\d+) (minute|second)s?(?: and (\d+) seconds?)?$/;
			if(r.test(str)) {
				const m = r.exec(str);

				return {
					seconds: Number(m[1]) * (m[2] === "minute" ? 60 : 1) + (m[3] ? Number(m[3]) : 0)
				};
			}
		},
		startTimerStarted: function(str) {
			const r = /^Queued the match to start in (\d+) (minute|second)s?(?: and (\d+) seconds?)?$/;
			if(r.test(str)) {
				const m = r.exec(str);
		...
		startTimerAborted: function(str) {
			return str == "Aborted the match start timer";
		},
		startTimerTick: function(str) {
			const r = /^Match starts in (\d+) (minute|second)s?(?: and (\d+) seconds?)?$/;
			if(r.test(str)) {
				const m = r.exec(str);
