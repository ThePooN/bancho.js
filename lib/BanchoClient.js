...
class BanchoClient extends EventEmitter {
	...
	constructor(optionsOrUsername = {}, password, host = "irc.ppy.sh", port = 6667, apiKey, limiterTimespan, limiterPrivate, limiterPublic) {
		...
		if(optionsOrUsername != null && typeof optionsOrUsername == "string" && password != null && typeof password == "string") {
			...
			this.rateLimiter = new RateLimiter({ tokensPerInterval: limiterPrivate, interval: limiterTimespan });
		}
			...
			if(options.rateLimiter === undefined) {
				...
				const limiterPrivate = options.botAccount ? 300 : options.limiterPrivate;
				const limiterTimespan = options.botAccount ? 60000 : options.limiterTimespan;
				options.rateLimiter = new RateLimiter({ tokensPerInterval: limiterPrivate, interval: limiterTimespan });
			}
