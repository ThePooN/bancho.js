...
const BanchoLobbyPlayerStates = require("./BanchoLobbyPlayerStates");
const BanchoLobbyTeams = require("./BanchoLobbyTeams");
const Regexes = require("./BanchoLobbyRegexes");
const Teams = require("./BanchoLobbyTeams");
...
class BanchoLobby extends EventEmitter {
			...
			switch(regex.name) {
				...
				case "beatmapFromSettings":
					...
					this.banchojs.osuApi.beatmaps.getByBeatmapId(this.beatmapId)
						.then((beatmap) => this.beatmap = beatmap)
				...
				case "playerFinished":
					this.getPlayerByName(ret.username).then((player) => {
						player.score = new BanchoLobbyPlayerScore(ret.score, ret.pass);
					}, (err) => this.banchojs.emit("error", err));
					break;
				case "matchFinished":
				case "matchAborted":
				...
				case "matchSize":
					/**
					 * @event BanchoLobby#matchSize
					 * @type {number}
					 */
					this.emit("matchSize", ret.size);
					break;
				case "matchSettings":
					/**
	 ...
	 * @param {number} [gamemode=nodesu.Mode.osu] See nodesu.Mode
	 * @async
	 */
	setMap(map, gamemode = nodesu.Mode.osu) {
		return new Promise((resolve, reject) => {
			...
			this.channel.sendMessage("!mp map "+map+" "+gamemode).catch(reject);
			const validMapListener = () => {
				resolve();
				this.removeListener("beatmapId", validMapListener);
				this.removeListener("invalidBeatmapId", invalidMapListener);
			};
			this.on("beatmapId", validMapListener);
	...
	setMods(mods, freemod = false) {
		return new Promise((resolve, reject) => {
			...
			const validModListener = () => {
				resolve();
				this.removeListener("mods", validModListener);
			};
			this.on("mods", validModListener);
		});
	}
	...
	lockSlots() {
		return new Promise((resolve, reject) => {
			this.channel.sendMessage("!mp lock").catch(reject);
			const listener = () => {
				resolve();
	...
	unlockSlots() {
		return new Promise((resolve, reject) => {
			this.channel.sendMessage("!mp unlock").catch(reject);
			const listener = () => {
				resolve();
	...
	setSize(size) {
		return new Promise((resolve, reject) => {
			...
			this.channel.sendMessage("!mp size "+size).catch(reject);
			const listener = (newSize) => {
				if(size == newSize) {
					resolve();
					this.removeListener("matchSize", listener);
				}
			};
			this.on("matchSize", listener);
		});
	}
	...
	setSettings(teamMode = this.teamMode, winCondition = this.winCondition, size) {
		return new Promise((resolve, reject) => {
			if(isNaN(teamMode) || isNaN(winCondition) || size != null && isNaN(size))
				return reject(new Error("one of the arguments isn't a number. try again"));
			
			this.channel.sendMessage("!mp set "+teamMode+" "+winCondition+((size != null) ? " "+size : "")).catch(reject);
	...
	movePlayer(player, slot) {
		return new Promise((resolve, reject) => {
			...
			this.channel.sendMessage("!mp move #"+player.id+" "+(slot+1)).then(resolve, reject);
		});
	}
	...
	invitePlayer(player) {
		return new Promise((resolve, reject) => 
			this.channel.sendMessage("!mp invite #"+player.id).then(resolve, reject)
		);
	}
	...
	setHost(player) {
		return new Promise((resolve, reject) => 
			this.channel.sendMessage("!mp host #"+player.id).then(resolve, reject)
		);
	}
	...
	kickPlayer(player) {
		return new Promise((resolve, reject) => 
			this.channel.sendMessage("!mp kick #"+player.id).then(resolve, reject)
		);
	}
	...
	clearHost() {
		return new Promise((resolve, reject) => {
			this.channel.sendMessage("!mp clearhost").catch(reject);
			const listener = () => {
				resolve();
	...
	closeLobby() {
		return new Promise((resolve, reject) => {
			this.channel.sendMessage("!mp close").catch(reject);
	...
	startMatch() {
		return new Promise((resolve, reject) => {
			this.channel.sendMessage("!mp start").catch(reject);
			const listener = () => {
				resolve();
	...
	abortMatch() {
		return new Promise((resolve, reject) => {
			this.channel.sendMessage("!mp abort").catch(reject);
			const listener = () => {
				resolve();
	...
	changeTeam(player, team) {
		return new Promise((resolve, reject) => {
			...
			this.channel.sendMessage("!mp team #"+player.id+" "+team).then(resolve, reject);
		});
	}
	...
	updateSettings() {
		...
		return this.updateSettingsPromise = new Promise((callerResolve, callerReject) => {
			this.pushSlotsUpdateQueue(new Promise((resolve, reject) => {
				this.channel.sendMessage("!mp settings");
